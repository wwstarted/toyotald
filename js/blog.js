document.addEventListener("DOMContentLoaded", () => {
  ...
  const videoTarget = document.getElementById("cdaVideoSettledArea");
  ...
  function settleVideo() {
    ...
    const from = videoIntro.getBoundingClientRect();
    const to = videoTarget.getBoundingClientRect();
    ...
    videoIntro.style.top = `${from.top}px`;
    videoIntro.style.left = `${from.left}px`;

    requestAnimationFrame(() => {
      videoIntro.style.transition =
        "transform 1s ease, width 1s ease, height 1s ease";
      videoIntro.style.transform = `translate(${translateX}px, ${translateY}px) scale(${scaleX}, ${scaleY})`;
    });

    setTimeout(() => {
      videoIntro.classList.add("is-settled");
      videoTarget.classList.add("has-video");
      isSettled = true;
    }, 1000);
  }

  window.addEventListener("scroll", () => {
    const trigger = videoTarget.getBoundingClientRect().top;
    if (trigger < window.innerHeight * 0.6) {
      settleVideo();
    }
  });
});

// NEW: Floating Dots Background
function initFloatingDots() {
  const container = document.getElementById("cdaFloatingDots");
  if (!container) return;
  ...
  for (let i = 0; i < dotCount; i++) {
    ...
    dot.className = "cda-dot";

    const startX = Math.random() * 120 - 10; // Có thể ra ngoài một chút
    ...
    dot.style.opacity = 0.4 + Math.random() * 0.3;
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    container.style.transform = `translate(${x * 0.05 - 5}%, ${y * 0.05 - 5}%)`;
  });
}

// NEW: Auto Sliding Subtitles
function initSubtitleSlider() {
  const slider = document.getElementById("cdaHeroSubtitles");
  if (!slider) return;

  const items = slider.querySelectorAll(".cda-subtitle-item");
  let current = 0;
  ...
  // Bắt đầu sau khi title hiện xong
  setTimeout(() => {
    setInterval(next, 3800); // Chuyển mỗi 3.8s
  }, 1500);
}
// Add to init()
document.addEventListener("DOMContentLoaded", () => {
